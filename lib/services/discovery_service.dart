...
class DiscoveryService {
  ...
  final Map<String, PSDevice> _discoveredDevices = <String, PSDevice>{};
  ...
  Future<void> startDiscovery({
    ...
    try {
      ...
      if (continuous) {
        ...
        _stopTimer = Timer(timeout, stopDiscovery);
      }

      _logger.i('Discovery started (continuous: $continuous)');
    } catch (error) {
      _logger.e('Failed to start discovery', error: error);
  ...
  Future<PSDevice?> probeHost(
    String host, {
    Duration timeout = const Duration(seconds: 3),
  }) async {
    final target = await _resolveHost(host);
    if (target == null) {
      return null;
    }
    ...
    final completer = Completer<PSDevice?>();
    ...
    subscription = socket.listen((event) {
      ...
      try {
        ...
        final device = _parseDiscoveryResponse(datagram.address.address, response);
        if (device != null) {
          completer.complete(device);
        }
      } catch (error) {
        _logger.w('Failed to parse direct probe response', error: error);
      }
    });
  ...
  Future<bool> wakeDevice(PSDevice device) async {
    ...
    try {
      ...
      _logger.e('Failed to wake device', error: error);
      return false;
    }
  }
  ...
  Future<List<String>> _resolveDiscoveryTargets() async {
    ...
    try {
      ...
      _logger.w('Failed to resolve directed broadcast address', error: error);
    }
  ...
  void _handleDatagram(RawSocketEvent event) {
    ...
    try {
      ...
      final previous = _discoveredDevices[device.hostId];
      final changed =
          previous == null ||
          previous.ipAddress != device.ipAddress ||
          previous.hostName != device.hostName ||
          previous.stateValue != device.stateValue ||
          previous.deviceTypeValue != device.deviceTypeValue ||
          previous.systemVersion != device.systemVersion;
      ...
      _logger.w('Failed to parse discovery response', error: error);
    }
  }
  ...
  Future<InternetAddress?> _resolveHost(String host) async {
    ...
    try {
      ...
      _logger.w('Failed to resolve host $host', error: error);
    }
